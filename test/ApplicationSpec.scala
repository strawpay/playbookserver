...
class ApplicationSpec extends PlaySpec with OneAppPerSuite {
  ...
  "Rocannon" should {
    ...
    "run a playbook with success" in {
      ...
      contentAsString(result) must startWith(s"""{"status":"success","buildNumber":"$buildNumber","message":"-vvvv -i test/resources/inventory -e {'version':'1.0'} --vault-password-file""")
